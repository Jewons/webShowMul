 ...
 * @param {} data
 */
function bandwithAdaptive_zbView(){
// 路径配置
		require.config({
			paths : {
		...
		function(ec) {
			...
			option1 = {
				...
				tooltip : {
	                ...
	                formatter : function (params) {
	                	...
	                	if(params[0].value[1]==1){
	                		return "道路切换，"+switchinfoMap.get(params[0].name+"_1_zb");
	                	}
	                	if(params[0].value[1]==2){
	                		return "浪涌，"+switchinfoMap.get(params[0].name+"_2_zb");
	                	}
	                },
	                textStyle:{
						align:'left'
				...
				xAxis : [ {
					...
					data:pathSwitch_audioOverFlowTime_zb
					//data:scatterTime
				} ],
				yAxis : [ {
					type : 'value',
				...
				series : [ {
					...
					symbolSize : 10,
					data:(function(){
		            	var oldArr = pathSwitch_audioOverFlowData_zb;
			...
			option2 = {
				tooltip : {
					//backgroundColor:'rgba(0,0,0,0)',//背景色透明，这样加了之后，再将formatter的返回值设置为""，就不会弹出标签。
					trigger : 'axis',//axis,item
					...
					formatter:function(params){
						...
						var adjust_count = BandwidthMap_zb.get(params[0].name);
				...
				series : [
						{
							...
							name : '冗余带宽',
							type : 'bar',
							data : redund_up_bwYArr_zb
						},{
							name : '有效上行带宽',
							type : 'bar',
							data : valid_up_bwYArr_zb
						},{
							name : 'FEC后音频带宽',
							type : 'bar',
							data : audio_bw_fYArr_zb
						},{
							name : 'FEC后视频带宽',
							type : 'bar',
							data : video_bw_fYArr_zb
						},{
							name : '自适应调节点',
							type : 'scatter',
							symbol: 'star',
				...
				series : [
						{
							name:'通话等级',
							type:'bar',
							data : trans_level_idArr_zb
						},{
							name:'视频fec等级',
							type:'bar',
							data : video_redundent_levArr_zb
						},{
							name:'音频fec等级',
							type:'bar',
							data : audio_redundent_levArr_zb
						}
				]
			};
			...
			option4 = {
				tooltip : {
					...
					formatter:function(params){
						...
						var audio_videoCodeRate = audio_videoMap_zb.get(params[0].name);
				...
				series : [ {
					...
					type : 'line',
					symbol : 'star',
					data : video_lose_bYArr_zb
					
				} ,{
					name : '视频fec纠错后丢包率',
					type : 'line',
					symbol : 'star',
					data : video_lose_fYArr_zb
					
				},{
					name : '音频原始丢包率',
					type : 'line',
					symbol : 'star',
					data : audio_lose_bYArr_zb
					
				} ,{
					name : '音频fec纠错后丢包率',
					type : 'line',
					symbol : 'star',
					data : audio_lose_fYArr_zb
				}]
			};
			...
			option5 = {
				tooltip : {
					...
					formatter:function(params){
						...
						var delay = delayMap_zb.get(params[0].name);
			...
			var myChart1 = ec.init(document.getElementById('bandAdaptive_main1_zb'));
			var myChart2 = ec.init(document.getElementById('bandAdaptive_main2_zb'));
			var myChart3 = ec.init(document.getElementById('bandAdaptive_main3_zb'));
			var myChart4 = ec.init(document.getElementById('bandAdaptive_main4_zb'));
			var myChart5 = ec.init(document.getElementById('bandAdaptive_main5_zb'));
			// 为echarts对象加载数据 
